...
export function getModelOption(id: ModelOptionId): ModelOption | undefined {
  return modelOptions.find((option) => option.id === id);
}

export function getModelOptionByProviderModel(provider: ProviderId, model: string): ModelOption | undefined {
  return modelOptions.find((option) => option.provider === provider && option.model === model);
}
