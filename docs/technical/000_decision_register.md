...
| TDR-086 | Gemini default with OpenAI option | Keep Gemini Flash-Lite as the global default while exposing OpenAI `gpt-4.1-nano` as an optional provider | Superseded | Superseded by TDR-087: first smoke stability is useful, but a visible/default model creates hierarchy between engines and weakens parallel model choice | 2026-06-03 |
| TDR-087 | Explicit parallel model choice | Do not present any model as the default; users must explicitly choose exactly one model before an attempt starts | Accepted | Model choice should feel like selecting a programming language before solving a problem: required, parallel, and independent from solving mode, with no privileged engine implied by UI defaults | 2026-06-03 |
...
| TDR-092 | Derived overlay index | Build graph overlay data as a sparse derived index from `ConversationGraph.annotations`, not as a new canonical source | Accepted | Fast and accurate overlay rendering should leverage existing node/edge/pair/annotation indexes while preserving trace as canonical and annotations as the auditable judgment layer | 2026-06-04 |
| TDR-093 | `.skai` file format | Implement `.skai v1` as canonical JSON with section-level SHA-256 hashes and embedded graph/report snapshots | Accepted | SKAI needs a portable orchestration artifact format before plugins, share replay, portfolio, or certification layers; JSON keeps v1 inspectable while hashes create tamper evidence without overclaiming authorship | 2026-06-04 |
