...
- Judge는 LLM mode를 켤 수 있고, Gemini judge key는 사용자 풀이용 key와 분리됐다. Golden calibration runner는 있으나, 현재 heuristic judge는 일부 weak/average 차이가 둔감하다. 다음 judge 강화는 `docs/technical/judge_research/002_judge_rubric_plan.md`의 Evidence Packet Builder -> Judge Output Schema V1 -> Gemini LLM Judge Prompt V2 순서로 진행해야 한다.
