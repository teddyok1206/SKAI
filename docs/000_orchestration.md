...
- language system 076-078이 완료됐다. `docs/design/004_copy_inventory.md`는 copy posture를 분류하고, `lib/i18n/copy-registry.json`은 35개 high-priority entry를 가진다. `npm run i18n:inventory`, `npm run i18n:check`, `npm run i18n:update`, `npm run i18n:draft`, `npm run verify:i18n`이 있다.
- i18n registry는 한 locale이 먼저 수정되면 반대 locale을 `stale` 또는 `missing`으로 표시하는 운영을 전제로 한다. `LanguageToggle` 컴포넌트와 localStorage 기반 explicit locale preference hook은 준비됐지만 아직 route UI에 붙이지 않았다.
...
- 다음 구현: topbar/home/problem browser copy를 registry 기반으로 렌더링하고 `LanguageToggle`을 실제 topbar에 붙인다.
- 완료 조건: `verify:i18n`이 missing/stale/protected-term drift를 검출하고, 주요 route의 copy가 locale registry에서 렌더링된다. (`verify:i18n` baseline 완료, route 렌더링 후속)
