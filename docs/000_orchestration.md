...
- 3D Dual view는 row-lane table이 아니라 Prompt spine, Response spine, Status spine의 3-spine layout으로 표현한다. Status는 항상 오른쪽 세 번째 spine에 놓이며, 중간 causal node가 아니라 각 prompt-response local cell을 묶는 세 번째 graph/binding layer다.
- 3D Dual view에서 `R_i`는 `P_i`와 같은 높이의 chat bubble이 아니라 `P_i -> P_{i+1}` prompt transition edge의 dual node다. 따라서 `R_i`와 `S_i`는 `P_i`와 `P_{i+1}`의 중점 높이에 놓인다. Dual relation은 사선 node-to-node shortcut이 아니라 horizontal ladder rung으로 표현한다: `P_i` node는 `R_{i-1}->R_i` response edge midpoint와 연결되고, `P_i->P_{i+1}` prompt edge midpoint는 `R_i` node와 연결된다.
